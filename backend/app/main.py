...
def create_app(settings: Settings | None = None, orchestrator: InvestigationOrchestrator | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        ws = WebSocketManager(settings.websocket_event_history_limit)
        manager = AnalysisManager(ws, settings.session_retention_seconds, settings.max_concurrent_analyses)
        app.state.settings = settings
        app.state.websocket_manager = ws
        app.state.analysis_manager = manager
        app.state.analysis_runner = AnalysisRunner(
            manager, orchestrator or FoundationOrchestrator(), settings.analysis_timeout_seconds
        )
        yield
