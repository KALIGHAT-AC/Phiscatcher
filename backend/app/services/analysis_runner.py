    ...
    async def run(self, analysis_id: str) -> None:
        try:
            async with self.manager.semaphore:
                ...
                cancel = await self.manager.cancellation_event(analysis_id)
                if cancel.is_set():
                    return
                ...
                await self.manager.transition(
                    ...
                    "Foundation orchestration started.",
                    "analysis_started",
                )
                try:
                    await asyncio.wait_for(self.orchestrator.run(session, cancel), timeout=self.timeout_seconds)
                except TimeoutError:
                    await self.manager.transition(
                        analysis_id,
                        AnalysisStatus.TIMEOUT,
                        "Analysis exceeded its configured timeout.",
                        "analysis_failed",
                        "Analysis timed out.",
                    )
                    ...
                    await self.manager.transition(
                        ...
                        "Analysis lifecycle failed.",
                        "analysis_failed",
                        "Unexpected analysis lifecycle failure.",
                    )
                else:
                    current = await self.manager.get(analysis_id)
                    if cancel.is_set() or current.status == AnalysisStatus.CANCELLED:
                        ...
                        await self.manager.transition(
                            ...
                            "Foundation orchestration completed; investigation components are not configured.",
                            "analysis_completed",
                        )
                finally:
